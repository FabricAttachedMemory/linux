...
struct posix_acl *tmfs_get_acl(struct inode *inode, int type)
{
	...
	size = tmfs_getxattr(inode, name, value, PAGE_SIZE);
	if (size > 0)
...
int tmfs_set_acl(struct inode *inode, struct posix_acl *acl, int type)
{
	...
	if (acl) {
		 ...
		 * permissions in the inode, if needed. tmfs_setxattr
		...
		ret = tmfs_setxattr(inode, name, value, size, 0);
		kfree(value);
	} else {
		ret = tmfs_removexattr(inode, name);
	}
