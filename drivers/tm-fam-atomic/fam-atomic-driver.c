...
#define SIZEOF_ATOMIC_REGISTER	65536
...
uint64_t lfs_offset_to_lza(int lfs_fd, int64_t offset)
{
	...
	/*
	 * Return error if LZA conversion did not succeed.
	 */
	if (ret)
		return (uint64_t)-1;
				 ...
				 unsigned int ioctl_num)
{
	...
	/*
	 * We must first cast the parameters to an unsigned type to prevent
	 * unintended sign extention when converting it to 128 bits.
	 */
			       ...
			       int64_t prev[2], unsigned int ioctl_num)
{
	...
	case FAM_ATOMIC_128_COMPARE_AND_STORE:
		param0 |= ((__uint128_t)((uint64_t)p128_1[1])) << 64;
		param0 |= ((__uint128_t)((uint64_t)p128_1[0]));
